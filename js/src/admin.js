var tabspanel = (function ($, undefined) {
        ...
        $.jmRequest({
            ...
            success: function (d) {
                if(d.status == 'success') {
                    ...
                    $.jmRequest({
                        handler: "ajax",
                        url: controller+'&mod_action=getImages',
                        method: 'get',
                        success: function (d) {
                            $('.block-img').empty();
    ...
    function initDefaultImg(controller) {
        $('.make_default').off().on('click', function(){
            ...
            $.jmRequest({
                handler: "ajax",
                url: controller+'&mod_action=setImgDefault',
                data: {id_img: id},
                method: 'post',
                success: function (d) {
                    if(!d.status) {
        ...
        $('#gallery-pages .sortable').off().on('change',function(){
            ...
            if(!dflt.length) {
                $.jmRequest({
                    handler: "ajax",
                    url: controller+'&mod_action=getImgDefault',
                    method: 'get',
                    success: function (d) {
    ...
    function initSortable(controller) {
        $( ".row.sortable" ).sortable({
            ...
            update: function(){
                ...
                $.jmRequest({
                    handler: "ajax",
                    url: controller+'&mod_action=orderImages',
                    method: 'POST',
                    data : serial,
                    success:function(e){
                        $.jmRequest.initbox(e,{
        ...
        run: function(controller){
            $( ".ui-sortable" ).sortable({
                ...
                update: function(){
                    ...
                    $.jmRequest({
                        handler: "ajax",
                        url: controller+'&mod_action=order',
                        method: 'POST',
                        data : serial,
                        success:function(e){
                            $.jmRequest.initbox(e,{
